 ...
 *
 * The engine will read/write data to Zustand stores during game state changes.
 */
class PlinkoEngine {
  /**
  ...
  /**
   * Unsubscribe function for store subscriptions - prevents memory leaks
   */
  private unsubscribePlinko: (() => void) | null = null;
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    // Get fresh state from stores (avoid caching stale references)
    const plinkoState = usePlinkoStore.getState();
    const commonState = useCommonStore.getState();

    this.betAmount = plinkoState.betAmount;
    this.rowCount = plinkoState.rowCount;
    this.riskLevel = plinkoState.riskLevel;
    this.balance = commonState.balance;

    // Subscribe to store updates and store unsubscribe function
    this.unsubscribePlinko = usePlinkoStore.subscribe((state) => {
      ...
      this.balance = useCommonStore.getState().balance;
    });
  ...
  /**
   * Stops (pauses) the rendering and physics engine, and cleans up subscriptions.
   */
  stop() {
    ...
    if (this.unsubscribePlinko) {
      this.unsubscribePlinko();
      this.unsubscribePlinko = null;
    }
  }
  ...
  dropBall() {
    ...
    (ball as PlinkoBall).betAmount = this.betAmount;
  ...
  private handleBallEnterBin(ball: Matter.Body) {
    ...
    if (binIndex !== -1 && binIndex < this.pinsLastRowXCoords.length - 1) {
      ...
      const ballBetAmount = (ball as PlinkoBall).betAmount || this.betAmount;
