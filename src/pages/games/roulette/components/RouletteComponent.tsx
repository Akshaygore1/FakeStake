...
import { type BetType, useRouletteStore } from "@/stores/game/roulette.store";
import { useCommonStore } from "@/stores/common.store";
import { isRedNumber } from "@/lib/roulette-constants";
import { toast } from "sonner";

function RouletteComponent() {
  const { balance, setBalance } = useCommonStore();
  const { currentBets, clearBets, addGameResult, isSpinning } =
    useRouletteStore();

  const checkBetWin = (bet: BetType, winningNumber: number): boolean => {
    switch (bet.type) {
      case "number":
      ...
      case "type":
        ...
        return false;
      case "color":
        if (bet.value === "red") return isRedNumber(winningNumber);
        if (bet.value === "black")
          return winningNumber > 0 && !isRedNumber(winningNumber);
        return false;
      case "column":
  ...
  const handleSpinComplete = (winningNumber: number) => {
    ...
    // Show results using toast instead of blocking alert
    if (totalWin > 0) {
      setTimeout(() => {
      ...
      setTimeout(() => {
        toast.error(`You lost $${totalBet.toFixed(2)}`, {
