...
export default function DiceComponent({
  ...
  // Use the audio hook instead of module-level Audio object
  const audioUrls = useMemo(() => [`${url}/drag-audio.mp3`], []);
  ...
  const handleValueChange = useCallback((newValue: number[]) => {
    ...
    play(`${url}/drag-audio.mp3`);
  }, [setValue, setMultiplier, setWinChance, play]);
          ...
          {result
            ...
            .map(
              (
                ...
                <div key={`dice-result-${item.randomNumber}-${item.isWin}-${index}`} className="flex-shrink-0">
                  <div
